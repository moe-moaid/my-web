  ...
  return (
    <div className='flex flex-row justify-center items-start gap-x-8 mt-8'>
      <form className="w-1/3 rounded-3xl bg-white max-h-fit flex flex-col items-center gap-y-4 px-4 py-8">
        ...
        <textarea className='w-full resize-none outline-none border-b border-[#2EC4CE]' rows={4} name="description" placeholder='Project description' />
        <div className="">
          <label htmlFor="project_images" className='bg-[#2E8CFA] text-white font-semibold px-2 py-1 rounded-md'>
            Upload Project Images
          </label>
          <input id='project_images' type="file" hidden multiple onChange={handleImagesUpload} accept='.jpg, .jpeg, .png'/>
        </div>
        <div>
          {!images && Image_Placeholders}
          {images && <img src={images} className='w-[57px] h-[54px] rounded-lg'/>}
        </div>
      </form>
      <form className="w-1/3 rounded-3xl bg-white flex flex-col items-center gap-y-4 px-4 py-8">
        <h1 className="text-center my-4 font-medium">Edit your Existing Projects</h1>
      </form>
    </div>
  )
}